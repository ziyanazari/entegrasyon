...
run("pm2 start all", "pm2 start all")
run("pm2 restart ikas-entegrasyon --update-env", "pm2 restart ikas-entegrasyon --update-env")
time.sleep(5)
run("pm2 status", "pm2 status")

client.close()
print("\nKRITIK HATALAR DUZELTILDI VE SISTEM YAYINDA!")
