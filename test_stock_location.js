...
async function test() {
  ...
  const getBranchQuery = `query { listStockLocation { id name } }`;
  ...
  console.log('Stock Location Response:', JSON.stringify(branchData, null, 2));

  // let's also query listProducts properly
  ...
  if (!prodData.data || !prodData.data.listProducts) {
    console.log('Product error:', JSON.stringify(prodData));
  } else {
