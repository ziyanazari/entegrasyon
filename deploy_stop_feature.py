...
run("static copy", f"cd {DIR} && cp -rf public .next/standalone/public; cp -rf .next/static .next/standalone/.next/static; echo ok")
run("pm2 start all", f"pm2 start all") # Onceden durdurmustuk, simdi hepsini aciyoruz
run("pm2 restart ikas-entegrasyon --update-env", f"pm2 restart ikas-entegrasyon --update-env")
time.sleep(5)
run("pm2 status", "pm2 status")

client.close()
print("\nDURDURMA OZELLIGI VE IYILESTIRMELER YAYINLANDI!")
