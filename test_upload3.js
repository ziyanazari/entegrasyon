...
async function test() {
  ...
  const body = newSearchParams = new URLSearchParams({
  ...
  // let's query listProduct properly using listProduct (NOT listProducts)
  ...
  if (!prodData.data || !prodData.data.listProduct) {
    console.log('Product error:', JSON.stringify(prodData));
  } else {
    // get first one
    const product = prodData.data.listProduct[0];
